...
class PublisherAgent:
    ...
    def run(self, state: PipelineState) -> PipelineState:
        ...
        webhook = os.getenv("SLACK_CONTENT_WEBHOOK_URL", os.getenv("SLACK_APPROVAL_WEBHOOK_URL", "")).strip()
        blocks = _format_slack_message(state)
        delivered = False
        if webhook:
            try:
                with httpx.Client(timeout=15) as client:
                    resp = client.post(webhook, json={"text": blocks["text"], "blocks": blocks["blocks"]})
