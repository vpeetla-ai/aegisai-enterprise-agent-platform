...
class PublisherAgent:
    ...
    def run(self, state: StockPipelineState) -> StockPipelineState:
        ...
        webhook = os.getenv("SLACK_STOCK_WEBHOOK_URL", os.getenv("SLACK_APPROVAL_WEBHOOK_URL", "")).strip()
        if webhook:
            try:
                with httpx.Client(timeout=15) as client:
                    client.post(webhook, json={"text": state.briefing_markdown[:3000]})
...
class StockResearchOrchestrator:
    SCHEDULE = "30 10 * * 1-5"  # 10:30 UTC ≈ 05:30 EST (weekdays)
    ...
    def posture(self) -> dict[str, object]:
        return {
            ...
            "agents": ["collector", "analyst", "anchor", "publisher"],
            "schedule": "Weekdays 05:30 AM EST",
